...
local function collectParts(origin,radius)
    ...
    for _,v in ipairs(WS:GetDescendants()) do
        if v:IsA("BasePart") then
            if (v.Position-origin).Magnitude<=radius then
                parts[#parts+1]=v
            end
        end
    end
...
Tog("Break Spawn Collisions",pgT,function() return C.BreakColl end,function(v)
    ...
    if v then
        ...
        local parts=collectParts(origin,C.CollRad)
...
local function destroyToyObj(toy)
...
local function destroyAllCoconuts()
    ...
    if folder then
        for _,t in ipairs(folder:GetChildren()) do
            if t.Name=="FoodCoconut" then destroyToyObj(t) end
        end
    end
    for toy in pairs(trackedToys) do destroyToyObj(toy) end
    trackedToys={}
    claimCooldown={}
end

local function claimToy(toy,ct)
    if not SNO or not toy or not toy.Parent then return end
    local last=claimCooldown[toy] or -999
    if (ct-last)<0.2 then return end
    claimCooldown[toy]=ct
...
task.spawn(function()
    ...
    DestroyToy=MT:FindFirstChild("DestroyToy")
        ...
        if anyCoco then
            ...
            if rt then
                ...
                if folder then
                    ...
                    if #aC<need and spCount%5==0 then
                        pcall(function()
                            SpawnToy:InvokeServer("FoodCoconut",rt.CFrame*CFrame.new(-5,5,10),Vector3.zero)
                        end)
                    end

                    local velOffset=CFrame.new(rt.Velocity/100)
...
KickLabel.BackgroundColor3=CL.Cd
KickLabel.Text="Kick: idle"
KickLabel.TextColor3=CL.Td
KickLabel.Font=Enum.Font.Gotham
KickLabel.TextSize=11
KickLabel.LayoutOrder=11
KickLabel.Parent=pgK
Instance.new("UICorner",KickLabel).CornerRadius=UDim.new(0,6)

local KS={Sel={}, Save=nil, Conn=nil}

do
local kGE=RSt:FindFirstChild("GrabEvents")
local kSNO=kGE and kGE:FindFirstChild("SetNetworkOwner")
local kDGL=kGE and kGE:FindFirstChild("DestroyGrabLine")

local function kInPlot(plr)
...
local function kFling(plr)
    ...
    if kInPlot(plr) then return end
    local myCh=LP.Character
    local myHrp=myCh and myCh:FindFirstChild("HumanoidRootPart")
    if not myHrp then return end
    if not KS.Save then KS.Save=myCh:GetPivot() end
    pcall(function()
        myCh:PivotTo(CFrame.new(hrp.Position+Vector3.new(5,-18.5,0)))
        for _,d in ipairs(ch:GetDescendants()) do
            if d:IsA("BasePart") then d.CanCollide=false end
        end
    end)
    if kSNO then pcall(function() kSNO:FireServer(hrp,hrp.CFrame) end) end
    task.wait(0.15)
    pcall(function() myCh:PivotTo(KS.Save) end)
    task.wait(0.05)
    if kDGL then pcall(function() kDGL:FireServer(hrp) end) end
    task.wait(0.1)
...
local function kTick()
    if not (C.LoopKick or C.KickAll) then return end
    if C.KickAll then
        for _,plr in ipairs(Pl:GetPlayers()) do
            if plr~=LP then kFling(plr) end
        end
    else
        for name in pairs(KS.Sel) do
            local plr=Pl:FindFirstChild(name)
            if plr then kFling(plr) end
        end
    end
end

local function kStart()
    if KS.Conn then KS.Conn:Disconnect() end
    KS.Save=nil
    KS.Conn=RS.Heartbeat:Connect(kTick)
end

local function kStop()
    if KS.Conn then KS.Conn:Disconnect() KS.Conn=nil end
    KS.Save=nil
end

local kList=Instance.new("ScrollingFrame")
kList.Size=UDim2.new(1,0,0,180)
kList.BackgroundColor3=CL.Ct
kList.BackgroundTransparency=0.3
kList.BorderSizePixel=0
kList.ScrollBarThickness=4
kList.ScrollBarImageColor3=CL.A
kList.CanvasSize=UDim2.new(0,0,0,0)
kList.AutomaticCanvasSize=Enum.AutomaticSize.Y
kList.LayoutOrder=12
kList.Parent=pgK
Instance.new("UICorner",kList).CornerRadius=UDim.new(0,6)
local kLL=Instance.new("UIListLayout",kList)
kLL.Padding=UDim.new(0,4)
kLL.SortOrder=Enum.SortOrder.Name
local kPd=Instance.new("UIPadding",kList)
kPd.PaddingTop=UDim.new(0,6)
kPd.PaddingLeft=UDim.new(0,6)
kPd.PaddingRight=UDim.new(0,6)
kPd.PaddingBottom=UDim.new(0,6)

local kRows={}
local function kRefresh()
    for _,r in pairs(kRows) do
    ...
    for _,plr in ipairs(Pl:GetPlayers()) do
        if plr~=LP then
            ...
            row.BackgroundColor3=KS.Sel[plr.Name] and CL.G or CL.Cd
            row.Text=plr.Name
            ...
            row.Activated:Connect(function()
                if KS.Sel[plr.Name] then
                    KS.Sel[plr.Name]=nil
                    row.BackgroundColor3=CL.Cd
                else
                    KS.Sel[plr.Name]=true
                    row.BackgroundColor3=CL.G
                end
            end)
            kRows[plr]={f=row}
        end
...
conns[#conns+1]=Pl.PlayerAdded:Connect(function() task.wait(0.1) kRefresh() end)
conns[#conns+1]=Pl.PlayerRemoving:Connect(function(plr)
    KS.Sel[plr.Name]=nil
    task.wait(0.1)
    kRefresh()
...
Tog("Loop Kick (selected)",pgK,function() return C.LoopKick end,function(v)
    C.LoopKick=v
    if v then
        C.KickAll=false
        KickLabel.Text="Kick: loop on selected"
        KickLabel.TextColor3=CL.R
        kStart()
    else
        if not C.KickAll then
            kStop()
            KickLabel.Text="Kick: idle"
            KickLabel.TextColor3=CL.Td
        end
...
Tog("Kick All",pgK,function() return C.KickAll end,function(v)
    C.KickAll=v
    if v then
        C.LoopKick=false
        KickLabel.Text="Kick: ALL players"
        KickLabel.TextColor3=CL.R
        kStart()
    else
        if not C.LoopKick then
            kStop()
            KickLabel.Text="Kick: idle"
            KickLabel.TextColor3=CL.Td
        end
...
Act("Stop",pgK,function()
    C.LoopKick=false
    C.KickAll=false
    kStop()
    KickLabel.Text="Kick: idle"
    KickLabel.TextColor3=CL.Td
end,15,Color3.fromRGB(90,45,45))

Act("Refresh List",pgK,function() kRefresh() end,16,CL.Ad)
end
