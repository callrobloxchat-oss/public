...
-- ============================================================
--   ЛОМАНИЕ ЧУЖОГО ДОМА (Troll)
-- ============================================================
Sec("ЛОМАНИЕ ЧУЖОГО ДОМА",pgT,70)

local BHKLabel=Instance.new("TextLabel")
BHKLabel.Size=UDim2.new(1,0,0,20)
BHKLabel.BackgroundColor3=CL.Cd
BHKLabel.Text="Ломание дома: выкл"
BHKLabel.TextColor3=CL.Td
BHKLabel.Font=Enum.Font.Gotham
BHKLabel.TextSize=11
BHKLabel.LayoutOrder=71
BHKLabel.Parent=pgT
Instance.new("UICorner",BHKLabel).CornerRadius=UDim.new(0,6)

local BHKInstr=Instance.new("TextLabel")
BHKInstr.Size=UDim2.new(1,0,0,44)
BHKInstr.BackgroundColor3=Color3.fromRGB(28,28,40)
BHKInstr.Text="Включите эту функцию, потом идите к постройке которую хотите разрушить"
BHKInstr.TextColor3=Color3.fromRGB(150,180,230)
BHKInstr.Font=Enum.Font.Gotham
BHKInstr.TextSize=10
BHKInstr.TextWrapped=true
BHKInstr.TextYAlignment=Enum.TextYAlignment.Center
BHKInstr.LayoutOrder=75
BHKInstr.Parent=pgT
Instance.new("UICorner",BHKInstr).CornerRadius=UDim.new(0,6)

do
local BHK_MenuToys=RSt:FindFirstChild("MenuToys")
local BHK_spawnRF=BHK_MenuToys and BHK_MenuToys:FindFirstChild("SpawnToyRemoteFunction")
local BHK_destroyToy=BHK_MenuToys and BHK_MenuToys:FindFirstChild("DestroyToy")
local BHK_GE=RSt:FindFirstChild("GrabEvents")
local BHK_SNO=BHK_GE and BHK_GE:FindFirstChild("SetNetworkOwner")

local BHK={On=false,anvilAng=0}

local function bhkNotify(txt,col,sec)
    BHKLabel.Text=txt
    BHKLabel.TextColor3=col or CL.O
    if sec then
        task.delay(sec,function()
            if BHKLabel.Text==txt then
                BHKLabel.Text=BHK.On and "Ломание дома: работает" or "Ломание дома: выкл"
                BHKLabel.TextColor3=BHK.On and CL.G or CL.Td
            end
        end)
    end
end

-- ==== Поиск свободного плота ====
local function bhkFindFreePlot()
    local plots=WS:FindFirstChild("Plots")
    if not plots then return nil end
    for _,plot in ipairs(plots:GetChildren()) do
        local owned=false
        for _,c in ipairs(plot:GetChildren()) do
            if (c:IsA("StringValue") or c:IsA("ObjectValue"))
                and c.Name:lower():find("owner") then
                if c:IsA("StringValue") and c.Value~="" then owned=true
                elseif c:IsA("ObjectValue") and c.Value~=nil then owned=true end
            end
        end
        if not owned then
            local sign=plot:FindFirstChild("Sign") or plot:FindFirstChild("PlotSign")
            if sign then
                local sg=sign:FindFirstChildOfClass("SurfaceGui") or sign:FindFirstChildOfClass("BillboardGui")
                if sg then
                    local txt=sg:FindFirstChildWhichIsA("TextLabel",true)
                    if txt and txt.Text~="" and txt.Text~="Unclaimed" and txt.Text~="Free"
                        and txt.Text~="Свободно" then
                        owned=true
                    end
                end
            end
        end
        if not owned then return plot end
    end
    return nil
end

local function bhkGetPlotCenter(plot)
    local sp=plot:FindFirstChild("Spawn") or plot:FindFirstChildWhichIsA("SpawnLocation",true)
...
local function bhkClaimPlot(plot)
    ...
    for _,r in ipairs(RSt:GetDescendants()) do
        if (r:IsA("RemoteEvent") or r:IsA("RemoteFunction")) then
            local n=r.Name:lower()
            if n:find("claim") or n:find("plot") or n:find("own") or n:find("house") then
                pcall(function() r:FireServer(plot) end)
                pcall(function() r:InvokeServer(plot) end)
            end
        end
    end
    return false
...
-- ==== Снежки ====
local function bhkGetSnowballs()
    local out={}
...
local function bhkSpawnSnowball(cf)
    if not BHK_spawnRF then return end
    cf=cf or CFrame.new(263.5,-4.5,486.9)
    task.spawn(function()
        pcall(function()
            BHK_spawnRF:InvokeServer("BallSnowball",cf,Vector3.new(0,-120.21099853515625,0))
        end)
    end)
end

local function bhkClaimSnowball(s)
    if not BHK_SNO then return end
    local sp=s:FindFirstChild("SoundPart")
    if sp then
        task.spawn(function()
            pcall(function() BHK_SNO:FireServer(sp,sp.CFrame) end)
        end)
    end
end

local function bhkTeleportSnowballs(list,cf)
...
local function bhkCleanupSnowballs()
    if not BHK_destroyToy then return end
    local f=WS:FindFirstChild(LP.Name.."SpawnedInToys")
    if f then
        for _,item in ipairs(f:GetChildren()) do
            if item.Name=="BallSnowball" then
                pcall(function() BHK_destroyToy:FireServer(item) end)
            end
        end
    end
end
...
-- ==== Anti Barrier ====
local function bhkSetAntiBarrier(val)
    local plots=WS:FindFirstChild("Plots")
    if not plots then return end
    for _,plot in ipairs(plots:GetChildren()) do
        local bm=plot:FindFirstChild("Barrier")
        if bm then
            for _,part in ipairs(bm:GetChildren()) do
...
-- ==== Наковальня (AnvilGray) ====
local function bhkFindMyAnvil()
    local f=WS:FindFirstChild(LP.Name.."SpawnedInToys")
    if not f then return nil end
    for _,c in ipairs(f:GetChildren()) do
        if c.Name=="AnvilGray" then return c end
    end
    return nil
end

local function bhkSpawnAnvil()
    if not BHK_spawnRF then return end
    local ch=LP.Character
    local hrp=ch and ch:FindFirstChild("HumanoidRootPart")
    if not hrp then return end
    local pos=hrp.CFrame*CFrame.new(4,2,0)
    pcall(function()
        BHK_spawnRF:InvokeServer("AnvilGray",pos,Vector3.new(0,0,0))
    end)
end

local function bhkRotateLoop()
    while BHK.On do
        task.wait(0.03)
        local ch=LP.Character
        local hrp=ch and ch:FindFirstChild("HumanoidRootPart")
        if hrp then
            local anvil=bhkFindMyAnvil()
            if anvil then
                BHK.anvilAng=BHK.anvilAng+0.08
                local rad=4
                local pos=hrp.Position+Vector3.new(
                    math.cos(BHK.anvilAng)*rad,
                    1.5,
                    math.sin(BHK.anvilAng)*rad
                )
                for _,d in ipairs(anvil:GetDescendants()) do
...
-- ==== Основной сценарий ====
local function bhkRun()
    bhkNotify("Ищу свободный дом...",CL.A)
    local plot=bhkFindFreePlot()
    if not plot then
        bhkNotify("Свободных домов нет!",CL.R,3)
        BHK.On=false
        return
    end
    bhkNotify("Дом: "..plot.Name,CL.A,2)

    local center=bhkGetPlotCenter(plot)
    if center then
        local ch=LP.Character
        local hrp=ch and ch:FindFirstChild("HumanoidRootPart")
        if hrp then hrp.CFrame=CFrame.new(center+Vector3.new(0,4,0)) end
    end
    task.wait(0.6)

    bhkNotify("Занимаю дом...",CL.A,2)
    bhkClaimPlot(plot)
    task.wait(1)

    bhkNotify("Ломаю барьер...",CL.O)
    bhkSetAntiBarrier(true)
    local t0=tick()
    local broken=false
    while (tick()-t0)<12 and BHK.On and not broken do
        local sb=bhkGetSnowballs()
        if #sb<2 then
            if #sb<1 then bhkSpawnSnowball() task.wait(0.02) end
            bhkSpawnSnowball()
            task.wait(0.02)
            sb=bhkGetSnowballs()
        end
        if #sb==2 then
            for _,s in ipairs(sb) do bhkClaimSnowball(s) end
            local barrier=plot:FindFirstChild("Barrier")
            local targetCf
            if barrier then
                local bp=barrier:FindFirstChildWhichIsA("BasePart")
                if bp then targetCf=CFrame.new(bp.Position) end
            end
            targetCf=targetCf or CFrame.new(264.58,-5.48,433.46)
            bhkTeleportSnowballs(sb,targetCf)
            local testCf=center and CFrame.new(center+Vector3.new(-20,-3,-10)) or nil
            if BHK_spawnRF then
                pcall(function()
                    BHK_spawnRF:InvokeServer("AnvilGray",
                        testCf or CFrame.new(242.66,-9.20,444.38),
                ...
                if f then
                    for _,c in ipairs(f:GetChildren()) do
                        if c.Name=="AnvilGray" then
                            local py=c:IsA("Model") and c:GetPivot().Position.Y or c.Position.Y
                            if py < -20 then broken=true end
                        end
                    end
                end
            end
    ...
    if broken then
        bhkNotify("Барьер сломан!",CL.G,2)
    else
        bhkNotify("AntiBarrier включён",CL.O,2)
    end
    task.wait(0.3)

    bhkNotify("Спавню наковальню...",CL.A,2)
    bhkSpawnAnvil()
    task.wait(0.8)
    bhkNotify(BHK.On and "Ломание дома: работает" or "Ломание дома: выкл",
        BHK.On and CL.G or CL.Td)
end

Tog("Ломание чужого дома",pgT,function() return BHK.On end,function(v)
    BHK.On=v
    if v then
        task.spawn(bhkRun)
        task.spawn(bhkRotateLoop)
    else
        bhkSetAntiBarrier(false)
        local anvil=bhkFindMyAnvil()
        if anvil then
            for _,d in ipairs(anvil:GetDescendants()) do
        ...
        bhkNotify("Ломание дома: выкл",CL.Td)
    end
end,72)
end
