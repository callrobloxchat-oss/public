...
local C={
    ...
    SpinGrab=false, SpinGrabSpeed=500,
    KickGrab=false,
    BreakColl=false, CollRad=600,
    BlackHole=false, BHSize=5, BHForce=100000,
}
...
local pgG=mkTab("Grab","G",2,true)
local pgB=mkTab("Bhop","B",3,false)
local pgO=mkTab("Other","O",4,false)
local pgT=mkTab("Troll","T",5,true)
local pgBl=mkTab("Blobman","B",6,true)
...
Tog("Anti Grab [BEST]",pgG,function() return _G.WM_AntiGrab end,function(v)
    _G.WM_AntiGrab=v
    if v then
        agCleanup()
...
--   SPIN GRAB
-- ============================================================
Sec("SPIN GRAB",pgG,20)

local spinGrabConn=nil

Tog("Spin Grab",pgG,function() return C.SpinGrab end,function(v)
    C.SpinGrab=v
    if v then
        if spinGrabConn then spinGrabConn:Disconnect() end
        spinGrabConn=WS.ChildAdded:Connect(function(e)
            if e.Name=="GrabParts" and e:FindFirstChild("GrabPart") then
                local dragPart=e:FindFirstChild("DragPart")
                if dragPart then
                ...
                if part1 then
                    task.spawn(function()
                        while WS:FindFirstChild("GrabParts") and C.SpinGrab and task.wait() do
                            pcall(function()
                                part1.AssemblyAngularVelocity=Vector3.new(0,C.SpinGrabSpeed,0)
                            end)
                        end
                    end)
                end
        ...
        if spinGrabConn then spinGrabConn:Disconnect() spinGrabConn=nil end
    end
end,21)

Sld("Spin Speed",pgG,10,1000,function() return C.SpinGrabSpeed end,function(v) C.SpinGrabSpeed=v end,22,10)

-- ============================================================
--   KICK GRAB
-- ============================================================
Sec("KICK GRAB",pgG,30)

local kickGrabConn=nil

Tog("Kick Grab",pgG,function() return C.KickGrab end,function(v)
    C.KickGrab=v
    if v then
        if kickGrabConn then kickGrabConn:Disconnect() end
        kickGrabConn=WS.ChildAdded:Connect(function(c)
            ...
            if Pl:FindFirstChild(part.Parent.Name) then
                local ge=RSt:FindFirstChild("GrabEvents")
                local dgl=ge and ge:FindFirstChild("DestroyGrabLine")
                local sno=ge and ge:FindFirstChild("SetNetworkOwner")
                if not dgl or not sno then return end
                while GrabPart and GrabPart.Parent and C.KickGrab do
                    pcall(function() dgl:FireServer(part) end)
                    RS.RenderStepped:Wait()
                    pcall(function() sno:FireServer(part,part.CFrame) end)
                end
            end
        end)
    else
        if kickGrabConn then kickGrabConn:Disconnect() kickGrabConn=nil end
    end
end,31)

-- ============================================================
--   BREAK SPAWN COLLISIONS + BLACK HOLE
-- ============================================================
Sec("BREAK SPAWN COLLISIONS",pgT,15)

local breakCollsConn=nil
local function collectParts(origin,radius)
    local parts={}
    for _,v in ipairs(WS:GetDescendants()) do
        if v:IsA("BasePart") and (v.Position-origin).Magnitude<=radius then
            parts[#parts+1]=v
        end
    end
    return parts
...
Tog("Break Spawn Collisions",pgT,function() return C.BreakColl end,function(v)
    C.BreakColl=v
    ...
    if v then
        ...
        for _,p in ipairs(collectParts(origin,C.CollRad)) do
            pcall(function() p.CanCollide=false p.CanTouch=false end)
        end
        breakCollsConn=WS.DescendantAdded:Connect(function(d)
            if C.BreakColl and d:IsA("BasePart") then
                pcall(function()
                    local c=LP.Character
                    local h=c and c:FindFirstChild("HumanoidRootPart")
                    if h and (d.Position-h.Position).Magnitude<=C.CollRad then
                        d.CanCollide=false d.CanTouch=false
                    end
                end)
            end
        end)
    else
        if breakCollsConn then breakCollsConn:Disconnect() breakCollsConn=nil end
    end
end,16)
Sld("Collision Radius",pgT,100,2000,function() return C.CollRad end,function(v) C.CollRad=v end,17,50)

Sec("BLACK HOLE",pgT,18)

local BHLabel=Instance.new("TextLabel")
BHLabel.Size=UDim2.new(1,0,0,20)
BHLabel.BackgroundColor3=CL.Cd
BHLabel.Text="BlackHole: idle"
BHLabel.TextColor3=CL.Td
BHLabel.Font=Enum.Font.Gotham
BHLabel.TextSize=11
BHLabel.LayoutOrder=19
BHLabel.Parent=pgT
Instance.new("UICorner",BHLabel).CornerRadius=UDim.new(0,6)

do
local BH_GE=RSt:FindFirstChild("GrabEvents")
local BH_SNO=BH_GE and BH_GE:FindFirstChild("SetNetworkOwner")
local BH={Part=nil, Attach=nil, BaseParts={}, Conn=nil, DescConn=nil}

local function bhForcePart(p)
    if not C.BlackHole then return end
    if not p or not p.Parent or not p:IsA("BasePart") then return end
    if p.Anchored or p==BH.Part then return end
    if p:IsDescendantOf(LP.Character) then return end
    if p.Name=="Handle" then return end
    if BH.BaseParts[p] then return end
    for _,c in ipairs(p:GetChildren()) do
        if c:IsA("BodyAngularVelocity") or c:IsA("BodyForce") or c:IsA("BodyGyro")
            or c:IsA("BodyPosition") or c:IsA("BodyThrust") or c:IsA("BodyVelocity")
            or c:IsA("RocketPropulsion") then
            pcall(function() c:Destroy() end)
        end
    end
    for _,n in ipairs({"Torque","AlignPosition","WM_Att"}) do
    ...
    tq.Name="Torque"
    tq.Torque=Vector3.new(C.BHForce,C.BHForce,C.BHForce)
    ...
    tq.Attachment0=at
    ap.Attachment0=at
    ap.Attachment1=BH.Attach
    tq.Parent=p ap.Parent=p at.Parent=p
    if BH_SNO then pcall(function() BH_SNO:FireServer(p,p.CFrame) end) end
    BH.BaseParts[p]=true
end

local function bhRelease(p)
    if not p or not p.Parent then return end
    pcall(function()
        local t=p:FindFirstChild("Torque") if t then t:Destroy() end
        local a=p:FindFirstChild("AlignPosition") if a then a:Destroy() end
        local at=p:FindFirstChild("WM_Att") if at then at:Destroy() end
        if p:IsA("BasePart") then p.CanCollide=true end
    end)
    BH.BaseParts[p]=nil
end

local function bhReleaseAll()
    for p in pairs(BH.BaseParts) do bhRelease(p) end
    BH.BaseParts={}
end

local function bhStart()
    if not BH.Part or not BH.Part.Parent then
        BH.Part=Instance.new("Part")
        BH.Part.Name="WM_BlackHole"
        BH.Part.Size=Vector3.new(C.BHSize,C.BHSize,C.BHSize)
        BH.Part.Transparency=1
        BH.Part.CanCollide=false
        BH.Part.CanQuery=false
        BH.Part.CanTouch=false
        BH.Part.Anchored=true
        BH.Part.Massless=true
        BH.Part.Parent=WS
        BH.Attach=Instance.new("Attachment")
        BH.Attach.Name="WM_BH_Attach"
        BH.Attach.Parent=BH.Part
    end
    local ch=LP.Character
    local hrp=ch and ch:FindFirstChild("HumanoidRootPart")
    if hrp then BH.Part.CFrame=hrp.CFrame end
    for _,p in ipairs(WS:GetDescendants()) do
        if p:IsA("BasePart") then bhForcePart(p) end
    end
    if BH.DescConn then BH.DescConn:Disconnect() end
    BH.DescConn=WS.DescendantAdded:Connect(function(d)
        if C.BlackHole and d:IsA("BasePart") then task.defer(function() bhForcePart(d) end) end
    end)
    if BH.Conn then BH.Conn:Disconnect() end
    BH.Conn=RS.Heartbeat:Connect(function()
        if not C.BlackHole then return end
        local c=LP.Character
        local h=c and c:FindFirstChild("HumanoidRootPart")
        if h and BH.Part and BH.Part.Parent then BH.Part.CFrame=h.CFrame end
    end)
end
...
local function bhStop()
    ...
    if BH.DescConn then BH.DescConn:Disconnect() BH.DescConn=nil end
    if BH.Part then BH.Part:Destroy() BH.Part=nil end
    BH.Attach=nil
end

Tog("Black Hole",pgT,function() return C.BlackHole end,function(v)
    C.BlackHole=v
    if v then
        BHLabel.Text="BlackHole: ON"
        BHLabel.TextColor3=CL.R
        bhStart()
    else
        bhStop()
        BHLabel.Text="BlackHole: idle"
...
Sld("BH Size",pgT,1,30,function() return C.BHSize end,function(v)
    C.BHSize=v
    if BH.Part then BH.Part.Size=Vector3.new(v,v,v) end
end,21,1)
Sld("BH Force",pgT,10000,1000000,function() return C.BHForce end,function(v) C.BHForce=v end,22,10000)
Act("Release All",pgT,function() bhReleaseAll() end,23,Color3.fromRGB(90,45,45))
end
    ...
    for _,plr in ipairs(players) do
        if plr~=LP then
            ...
            local function refresh()
                if fTgt==plr then
                    fb.BackgroundColor3=CL.G fb.Text="Active"
                else
                    fb.BackgroundColor3=CL.Ad fb.Text="Follow"
                end
                if orbitTarget==plr then
                    ob.BackgroundColor3=CL.G ob.Text="Orbit!"
                else
                    ob.BackgroundColor3=Color3.fromRGB(180,120,40) ob.Text="Orbit"
                end
            end
            refresh()
            fb.Activated:Connect(function()
                if fTgt==plr then sF() else
                    sF()
                    fTgt=plr
            ...
            tb.Activated:Connect(function()
                ...
                hrp.CFrame=CFrame.new(tpos.X,y,tpos.Z)*CFrame.Angles(0,math.rad(thrp.Orientation.Y),0)
            end)
...
Act("Break Barrier",pgT,function()
    task.spawn(function()
        bszNotify("Break Barrier: breaking...",3)
        ...
        bszCleanup()
        if broken then
            bszNotify("Break Barrier: broken + AntiBarrier ON",3)
        else
            bszNotify("Break Barrier: AntiBarrier ON (snowballs failed)",3)
        end
    end)
...
-- ============================================================
--   BLOBMAN TARGET (Select Player + Lock/Bring/Kick)
-- ============================================================
Sec("BLOBMAN TARGET",pgBl,10)

local BlobStatus=Instance.new("TextLabel")
BlobStatus.Size=UDim2.new(1,0,0,20)
BlobStatus.BackgroundColor3=CL.Cd
BlobStatus.Text="Blobman: idle"
BlobStatus.TextColor3=CL.Td
BlobStatus.Font=Enum.Font.Gotham
BlobStatus.TextSize=11
BlobStatus.LayoutOrder=11
BlobStatus.Parent=pgBl
Instance.new("UICorner",BlobStatus).CornerRadius=UDim.new(0,6)

local SelLabel=Instance.new("TextButton")
SelLabel.Size=UDim2.new(1,0,0,32)
SelLabel.BackgroundColor3=CL.Cd
SelLabel.Text="Select Player: none"
SelLabel.TextColor3=CL.Tx
SelLabel.Font=Enum.Font.GothamBold
SelLabel.TextSize=11
SelLabel.TextXAlignment=Enum.TextXAlignment.Left
SelLabel.AutoButtonColor=false
SelLabel.LayoutOrder=12
SelLabel.Parent=pgBl
Instance.new("UICorner",SelLabel).CornerRadius=UDim.new(0,6)
local SelPad=Instance.new("UIPadding",SelLabel)
SelPad.PaddingLeft=UDim.new(0,10)

local DropList=Instance.new("ScrollingFrame")
DropList.Size=UDim2.new(1,0,0,150)
DropList.BackgroundColor3=CL.Ct
DropList.BackgroundTransparency=0.15
DropList.BorderSizePixel=0
DropList.ScrollBarThickness=4
DropList.ScrollBarImageColor3=CL.A
DropList.CanvasSize=UDim2.new(0,0,0,0)
DropList.AutomaticCanvasSize=Enum.AutomaticSize.Y
DropList.Visible=false
DropList.LayoutOrder=13
DropList.Parent=pgBl
Instance.new("UICorner",DropList).CornerRadius=UDim.new(0,6)
local DL_Lay=Instance.new("UIListLayout",DropList)
DL_Lay.Padding=UDim.new(0,3)
DL_Lay.SortOrder=Enum.SortOrder.Name
local DL_Pad=Instance.new("UIPadding",DropList)
DL_Pad.PaddingTop=UDim.new(0,4)
DL_Pad.PaddingLeft=UDim.new(0,4)
DL_Pad.PaddingRight=UDim.new(0,4)
DL_Pad.PaddingBottom=UDim.new(0,4)

local BT={Sel=nil, Rows={}, Open=false}

local function btInPlot(plr)
    local pi=WS:FindFirstChild("PlotItems")
    local pip=pi and pi:FindFirstChild("PlayersInPlots")
    return pip and pip:FindFirstChild(plr.Name)~=nil
end

local function btSeated()
    local ch=LP.Character
    local hum=ch and ch:FindFirstChildOfClass("Humanoid")
    if not hum or not hum.Sit or not hum.SeatPart then return false end
    if not hum.SeatPart.Parent then return false end
    if hum.SeatPart.Parent.Name~="CreatureBlobman" then return false end
    return true
end

local function handleCreatureGrab(targetName, mode)
    local myChar=LP.Character
    if not myChar then return end
    local myHum=myChar:FindFirstChildOfClass("Humanoid")
    if not btSeated() or not myHum then
        BlobStatus.Text="Please, sit on any Blobman"
        BlobStatus.TextColor3=CL.O
        return
    end
    local seatParent=myHum.SeatPart.Parent
    local target=Pl:FindFirstChild(targetName)
    ...
    local creatureDrop=blobScript:FindFirstChild("CreatureDrop")
    if not creatureGrab then return end

    local grabParams={leftDet, tHRP, leftDet and leftDet:FindFirstChild("LeftWeld")}

    if mode==2 then
        pcall(function() creatureGrab:FireServer(unpack(grabParams)) end)
        task.wait(0.155)
        myHum.Sit=false
    elseif mode==3 then
        if target.Character:GetAttribute("Kicking") then return end
        target.Character:SetAttribute("Kicking",true)
        local function verifyInHand()
            if myHum.SeatPart and myHum.SeatPart.Parent=="CreatureBlobman" then
                local leftWeld=leftDet and leftDet:FindFirstChild("LeftWeld")
                local att=leftWeld and leftWeld.Attachment0
                if att and att.Parent and att.Parent.Parent==target.Character then
                    return true
                end
            end
            return false
        ...
        for _=1,50 do
            if not btSeated() then break end
            if verifyInHand() then
                if creatureDrop then
                    pcall(function() creatureDrop:FireServer(leftDet.LeftWeld, tHRP) end)
                end
                break
            end
            pcall(function() creatureGrab:FireServer(unpack(grabParams)) end)
        ...
        bv.Parent=tHRP
        task.delay(1.5,function() if bv.Parent then bv:Destroy() end end)
        target.Character:SetAttribute("Kicking",nil)
    else
        pcall(function() creatureGrab:FireServer(unpack(grabParams)) end)
    end
end

local function btRefresh()
    for _,r in pairs(BT.Rows) do
        if r.btn and r.btn.Parent then r.btn:Destroy() end
    end
    BT.Rows={}
    local players=Pl:GetPlayers()
    table.sort(players,function(a,b) return a.Name:lower()<b.Name:lower() end)
    for _,plr in ipairs(players) do
        if plr~=LP then
            ...
            row.BackgroundColor3=BT.Sel==plr.Name and CL.G or CL.Cd
            row.Text=plr.Name.." ("..plr.DisplayName..")"
            row.TextColor3=CL.Tx
            ...
            row.Activated:Connect(function()
                BT.Sel=plr.Name
                SelLabel.Text="Select Player: "..plr.Name
                BT.Open=false
                DropList.Visible=false
                for _,r in pairs(BT.Rows) do
                    if r.btn and r.btn.Parent then
                        r.btn.BackgroundColor3=(r.name==BT.Sel) and CL.G or CL.Cd
                    end
                end
            end)
            BT.Rows[plr]={btn=row, name=plr.Name}
        end
    end
end

SelLabel.Activated:Connect(function()
    BT.Open=not BT.Open
    DropList.Visible=BT.Open
    if BT.Open then btRefresh() end
end)

btRefresh()

conns[#conns+1]=Pl.PlayerAdded:Connect(function() task.wait(0.1) btRefresh() end)
conns[#conns+1]=Pl.PlayerRemoving:Connect(function(plr)
    if BT.Sel==plr.Name then
        BT.Sel=nil
        SelLabel.Text="Select Player: none"
    end
    task.wait(0.1)
...
Act("Lock",pgBl,function()
    if not BT.Sel then BlobStatus.Text="Select player first" BlobStatus.TextColor3=CL.R return end
    BlobStatus.Text="Locking: "..BT.Sel
    BlobStatus.TextColor3=CL.R
    handleCreatureGrab(BT.Sel,2)
end,20,CL.Ad)

Act("Bring",pgBl,function()
    if not BT.Sel then BlobStatus.Text="Select player first" BlobStatus.TextColor3=CL.R return end
    BlobStatus.Text="Bringing: "..BT.Sel
    BlobStatus.TextColor3=CL.G
    handleCreatureGrab(BT.Sel)
end,21,CL.G)

Act("Kick",pgBl,function()
    if not BT.Sel then BlobStatus.Text="Select player first" BlobStatus.TextColor3=CL.R return end
    BlobStatus.Text="Kicking: "..BT.Sel
    BlobStatus.TextColor3=CL.R
    handleCreatureGrab(BT.Sel,3)
end,22,Color3.fromRGB(200,60,60))

Act("Refresh List",pgBl,function() btRefresh() end,23,CL.Cd)
...
conns[#conns+1]=RS.Heartbeat:Connect(function(dt)
    ...
    if orbitTarget then
        ...
        elseif orbitTarget.Character and orbitTarget.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if orbitBG and orbitBG.Parent then orbitBG.CFrame=CFrame.new(pos,thrp.Position) end
        end
    end
