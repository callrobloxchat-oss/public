...
local C={
    ...
    BlobGrab=false, BlobAlternate=false,
}
...
-- ============================================================
--   BREAK SPAWN COLLISIONS + BLACK HOLE (Troll)
-- ============================================================
...
local BlobLabel=Instance.new("TextLabel")
BlobLabel.Size=UDim2.new(1,0,0,20)
BlobLabel.BackgroundColor3=CL.Cd
BlobLabel.Text="Blobman: idle"
BlobLabel.TextColor3=CL.Td
BlobLabel.Font=Enum.Font.Gotham
BlobLabel.TextSize=11
BlobLabel.LayoutOrder=11
BlobLabel.Parent=pgBl
Instance.new("UICorner",BlobLabel).CornerRadius=UDim.new(0,6)

local BS2={Sel={}, Rows={}, Conn=nil, Alter=1}

local function blInPlot(plr)
...
local function blGrab(plr)
    if not plr or not plr.Character then return false end
    local blob=blGetMyBlobman()
    if not blob then return false end
    local hrp=plr.Character:FindFirstChild("HumanoidRootPart")
    if not hrp then return false end
    if blInPlot(plr) then return false end
    local left=blob:FindFirstChild("LeftDetector")
    local right=blob:FindFirstChild("RightDetector")
    local script=blob:FindFirstChild("BlobmanSeatAndOwnerScript")
    if not script then return false end
    local remote=script:FindFirstChild("CreatureGrab")
    if not remote then return false end
    local det, weld
    if BS2.Alter==1 or not C.BlobAlternate then
        det=left weld=det and det:FindFirstChild("LeftWeld")
        BS2.Alter=2
    else
        det=right weld=det and det:FindFirstChild("RightWeld")
        BS2.Alter=1
    end
    if not det or not weld then return false end
    pcall(function() remote:FireServer(det,hrp,weld) end)
    return true
end
local function blDrop(plr)
    if not plr or not plr.Character then return false end
    local blob=blGetMyBlobman()
    if not blob then return false end
    local hrp=plr.Character:FindFirstChild("HumanoidRootPart")
    ...
    local left=blob:FindFirstChild("LeftDetector")
    local right=blob:FindFirstChild("RightDetector")
    if left and left:FindFirstChild("LeftWeld") then
        pcall(function() drop:FireServer(left.LeftWeld,hrp) end)
    end
    if right and right:FindFirstChild("RightWeld") then
        pcall(function() drop:FireServer(right.RightWeld,hrp) end)
    end
    return true
end
local function blTick()
    if not C.BlobGrab then return end
    local blob=blGetMyBlobman()
    if not blob then
        BlobLabel.Text="Blobman: not seated"
        BlobLabel.TextColor3=CL.O
        return
    end
    BlobLabel.Text="Blobman: grabbing selected"
    BlobLabel.TextColor3=CL.R
    for name in pairs(BS2.Sel) do
        local plr=Pl:FindFirstChild(name)
        if plr and plr.Character and plr~=LP then
            blGrab(plr)
        end
    end
end
local function blStart()
    if BS2.Conn then BS2.Conn:Disconnect() end
    BS2.Conn=RS.Heartbeat:Connect(function()
        if not C.BlobGrab then return end
        blTick()
    end)
end
local function blStop()
    if BS2.Conn then BS2.Conn:Disconnect() BS2.Conn=nil end
end

local blList=Instance.new("ScrollingFrame")
blList.Size=UDim2.new(1,0,0,260)
blList.BackgroundColor3=CL.Ct
blList.BackgroundTransparency=0.3
blList.BorderSizePixel=0
blList.ScrollBarThickness=6
blList.ScrollBarImageColor3=CL.A
blList.CanvasSize=UDim2.new(0,0,0,0)
blList.AutomaticCanvasSize=Enum.AutomaticSize.Y
blList.LayoutOrder=12
blList.Parent=pgBl
Instance.new("UICorner",blList).CornerRadius=UDim.new(0,6)
local blLL=Instance.new("UIListLayout",blList)
blLL.Padding=UDim.new(0,4)
blLL.SortOrder=Enum.SortOrder.Name
local blPd=Instance.new("UIPadding",blList)
blPd.PaddingTop=UDim.new(0,6)
blPd.PaddingLeft=UDim.new(0,6)
blPd.PaddingRight=UDim.new(0,6)
blPd.PaddingBottom=UDim.new(0,6)

local function blRefresh()
    for _,r in pairs(BS2.Rows) do
        if r.f and r.f.Parent then r.f:Destroy() end
    end
    BS2.Rows={}
    for _,plr in ipairs(Pl:GetPlayers()) do
        if plr~=LP then
            ...
            btn.Text=plr.Name
            btn.TextColor3=BS2.Sel[plr.Name] and CL.G or CL.Tx
            ...
            mark.Size=UDim2.new(0,22,1,0)
            mark.Position=UDim2.new(1,-24,0,0)
            mark.BackgroundTransparency=1
            mark.Text=BS2.Sel[plr.Name] and "✓" or ""
            mark.TextColor3=CL.G
            mark.Font=Enum.Font.GothamBold
            mark.TextSize=16
            mark.Parent=row
            btn.Activated:Connect(function()
                if BS2.Sel[plr.Name] then
                    BS2.Sel[plr.Name]=nil
                    btn.TextColor3=CL.Tx
                    mark.Text=""
                else
                    BS2.Sel[plr.Name]=true
                    btn.TextColor3=CL.G
                    mark.Text="✓"
                end
            end)
...
conns[#conns+1]=Pl.PlayerAdded:Connect(function() task.wait(0.1) blRefresh() end)
conns[#conns+1]=Pl.PlayerRemoving:Connect(function(plr)
    BS2.Sel[plr.Name]=nil
    task.wait(0.1)
    blRefresh()
...
Tog("Enable Grab Loop",pgBl,function() return C.BlobGrab end,function(v)
    C.BlobGrab=v
    if v then
        blStart()
    else
...
Tog("Alternate Hands",pgBl,function() return C.BlobAlternate end,function(v) C.BlobAlternate=v end,14)

Act("Grab Once",pgBl,function()
    local blob=blGetMyBlobman()
    if not blob then
        BlobLabel.Text="Blobman: not seated"
        BlobLabel.TextColor3=CL.O
    ...
    for name in pairs(BS2.Sel) do
        local plr=Pl:FindFirstChild(name)
        if plr then blGrab(plr) end
    end
end,15,CL.Ad)

Act("Drop All",pgBl,function()
    for name in pairs(BS2.Sel) do
        local plr=Pl:FindFirstChild(name)
        if plr then blDrop(plr) end
    end
end,16,Color3.fromRGB(90,45,45))

Act("Select All",pgBl,function()
    for _,plr in ipairs(Pl:GetPlayers()) do
        if plr~=LP then BS2.Sel[plr.Name]=true end
    end
    blRefresh()
end,17,CL.G)

Act("Clear Selection",pgBl,function()
    BS2.Sel={}
    blRefresh()
end,18,Color3.fromRGB(70,45,45))

Act("Refresh List",pgBl,function() blRefresh() end,19,CL.Cd)
