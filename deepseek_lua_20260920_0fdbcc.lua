...
-- ============================================================
--   DELETE BASEPLATE (Troll)
-- ============================================================
Sec("DELETE BASEPLATE",pgT,18)

local BPLabel=Instance.new("TextLabel")
BPLabel.Size=UDim2.new(1,0,0,20)
BPLabel.BackgroundColor3=CL.Cd
BPLabel.Text="Baseplate: idle"
BPLabel.TextColor3=CL.Td
BPLabel.Font=Enum.Font.Gotham
BPLabel.TextSize=11
BPLabel.LayoutOrder=19
BPLabel.Parent=pgT
Instance.new("UICorner",BPLabel).CornerRadius=UDim.new(0,6)

do
local BP_MT=RSt:FindFirstChild("MenuToys")
local BP_destroy=BP_MT and BP_MT:FindFirstChild("DestroyToy")
local BP_GE=RSt:FindFirstChild("GrabEvents")
local BP_sno=BP_GE and BP_GE:FindFirstChild("SetNetworkOwner")
local BP_dgl=BP_GE and BP_GE:FindFirstChild("DestroyGrabLine")

local function bpNotify(txt,col,sec)
    BPLabel.Text=txt
    BPLabel.TextColor3=col or CL.O
    if sec then
        task.delay(sec,function()
            if BPLabel.Text==txt then
                BPLabel.Text="Baseplate: idle"
                BPLabel.TextColor3=CL.Td
            end
        end)
...
Act("Delete Baseplate",pgT,function()
    task.spawn(function()
        bpNotify("Baseplate: finding...",CL.A)

        -- Ищем все "полы" — Baseplate, Floor, Ground, Terrain-замены
        local targets={}
        for _,v in ipairs(WS:GetChildren()) do
            if v:IsA("BasePart") and (
                ...
                or v.Name=="Ground"
                or v.Name=="Platform"
                or (v.Anchored and v.Size.X>200 and v.Size.Z>200 and v.Size.Y<50)
            ) then
                targets[#targets+1]=v
            end
        end

        -- Также проверяем Workspace.Map и Workspace.Plots
        for _,container in ipairs({WS:FindFirstChild("Map"),WS:FindFirstChild("Plots")}) do
            if container then
                for _,v in ipairs(container:GetDescendants()) do
                    if v:IsA("BasePart") and v.Anchored and v.Size.X>200 and v.Size.Z>200 and v.Size.Y<50 then
                        targets[#targets+1]=v
                    end
                end
            end
        ...
        if #targets==0 then
            bpNotify("Baseplate: not found",CL.R,3)
            return
        end

        bpNotify("Baseplate: found "..#targets,CL.A,2)

        -- МЕТОД 1: серверное удаление (реплицируется всем)
        for _,t in ipairs(targets) do
            if BP_destroy then
                for _=1,3 do
                    pcall(function() BP_destroy:FireServer(t) end)
                end
            end
            if BP_dgl then
                pcall(function() BP_dgl:FireServer(t) end)
            end
        end

        task.wait(0.5)

        -- МЕТОД 2: проверяем, остались ли цели
        local left=0
        for _,t in ipairs(targets) do
            if t.Parent then left=left+1 end
        end

        if left==0 then
            bpNotify("Baseplate: deleted (all see)",CL.G,3)
        else
            -- МЕТОД 3: fallback — убираем коллизию клиентски
            for _,t in ipairs(targets) do
                pcall(function()
                    if BP_sno then BP_sno:FireServer(t,t.CFrame) end
                    t.CanCollide=false
                    t.CanTouch=false
                    t.CFrame=CFrame.new(t.Position+Vector3.new(0,-500,0))
                end)
            end
            bpNotify("Baseplate: sunk "..left.." (client)",CL.O,3)
        end
    end)
end,20,Color3.fromRGB(200,60,60))
end
...
-- ============================================================
--   COCONUTS + COCOON (network-replicated to all)
-- ============================================================
local SNO=nil
local SpawnToy=nil
