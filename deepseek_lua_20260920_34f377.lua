...
--   AUTO-FALL ALL (Troll)
-- ============================================================
Sec("AUTO-FALL ALL",pgT,18)

local FallLabel=Instance.new("TextLabel")
FallLabel.Size=UDim2.new(1,0,0,20)
FallLabel.BackgroundColor3=CL.Cd
FallLabel.Text="Auto-Fall: idle"
FallLabel.TextColor3=CL.Td
FallLabel.Font=Enum.Font.Gotham
FallLabel.TextSize=11
FallLabel.LayoutOrder=19
FallLabel.Parent=pgT
Instance.new("UICorner",FallLabel).CornerRadius=UDim.new(0,6)

local fallConns={}
local fallChars={}
local savedFallHeight=WS.FallenPartsDestroyHeight

do
local FGE=RSt:FindFirstChild("GrabEvents")
local FSNO=FGE and FGE:FindFirstChild("SetNetworkOwner")

local function fallStop(char)
    fallChars[char]=nil
end

local function makeFall(char)
    if not char or not C.AutoFall then return end
    if fallChars[char] then return end
    fallChars[char]=true
    ...
    -- Сетевой владелец (постоянно)
    if FSNO then
        task.spawn(function()
            while C.AutoFall and char.Parent and hrp.Parent do
                pcall(function() FSNO:FireServer(hrp,hrp.CFrame) end)
                task.wait(0.2)
            end
        end)
    end

    -- Бессмертие (только у тебя локально)
    if hum then
        task.spawn(function()
            while C.AutoFall and hum.Parent do
                pcall(function()
                    if hum.MaxHealth<1e8 then hum.MaxHealth=1e9 end
    ...
    -- Постоянно вниз (с отбросом наверх перед зоной удаления)
    task.spawn(function()
        while C.AutoFall and char.Parent and hrp.Parent do
            pcall(function()
...
local function fallDisconnect()
    for _,c in ipairs(fallConns) do
        pcall(function() c:Disconnect() end)
    end
    fallConns={}
end

Tog("Auto-Fall All",pgT,function() return C.AutoFall end,function(v)
    C.AutoFall=v
    if v then
        FallLabel.Text="Auto-Fall: ON"
        FallLabel.TextColor3=CL.R
        pcall(function() WS.FallenPartsDestroyHeight=-1e9 end)

        for _,plr in ipairs(Pl:GetPlayers()) do
            if plr~=LP and plr.Character then
                makeFall(plr.Character)
            end
            if plr~=LP then
                fallConns[#fallConns+1]=plr.CharacterAdded:Connect(function(c)
                    if C.AutoFall then
                        task.wait(0.1)
                        makeFall(c)
                    end
        ...
        fallConns[#fallConns+1]=Pl.PlayerAdded:Connect(function(plr)
            ...
            if plr.Character then makeFall(plr.Character) end
        end)
    else
        FallLabel.Text="Auto-Fall: idle"
        FallLabel.TextColor3=CL.Td
        fallDisconnect()
        fallChars={}
        pcall(function() WS.FallenPartsDestroyHeight=savedFallHeight end)
    end
end,20)
end
